...
import { getUser, saveUser, getExpenses, getBudgets, Expense, Budget } from '../../lib/storage';
...
export default function Settings() {
  ...
  return (
    <ScrollView style={styles.container}>
              ...
              >
                <View>
                  ...
                  <Text style={styles.monthMeta}>
                    {monthExpenses.length} expense{monthExpenses.length !== 1 ? 's' : ''} · ${totalSpent.toFixed(2)} spent
                  </Text>
                </View>
                ...
                <View style={styles.monthDetail}>
                  ...
                  <View style={styles.summaryRow}>
                    ...
                    <View style={styles.summaryCard}>
                      ...
                      <Text style={[styles.sumAmount, { color: totalBudget - totalSpent >= 0 ? '#38a169' : '#e53e3e' }]}>
                  ...
                  {budgets.length > 0 && (
                    <>
                      ...
                      {budgets.map(b => {
                        const spent = spentByCategory(b.category, m.month, m.year);
                        const pct = b.amount === 0 ? 0 : Math.min(Math.round((spent / b.amount) * 100), 100);
                        const over = spent > b.amount;
                        return (
                          <View key={b.id} style={styles.goalRow}>
                            <View style={styles.goalHeader}>
                              <Text style={styles.goalCat}>{b.category}</Text>
                              <Text style={[styles.goalPct, { color: over ? '#e53e3e' : '#38a169' }]}>
                                {over ? 'Over budget' : `${pct}%`}
                              </Text>
                            </View>
                            <View style={styles.goalAmounts}>
                              <Text style={styles.goalSub}>${spent.toFixed(0)} / ${b.amount.toFixed(0)}</Text>
                            </View>
                            <View style={styles.progressBar}>
                              <View style={[styles.progressFill, { width: `${pct}%` as any, backgroundColor: over ? '#e53e3e' : '#333' }]} />
                            </View>
                          </View>
                        );
                      })}
                  ...
                  {monthExpenses.length === 0 ? (
                    <Text style={styles.noData}>No expenses recorded this month</Text>
                  ) : (
                    monthExpenses.map(e => (
                      <View key={e.id} style={styles.expRow}>
                        <View style={styles.expLogo}>
                          <Text style={styles.expLogoText}>{e.description.charAt(0).toUpperCase()}</Text>
                        </View>
                        ...
                        <View style={styles.expRight}>
                          <Text style={styles.expAmount}>${e.amount.toFixed(0)}</Text>
                          <Text style={styles.expDate}>{formatDate(e.date)}</Text>
                        </View>
                      </View>
                    ))
                  )}
      ...
      <TouchableOpacity style={styles.signOutBtn} onPress={() => router.replace('/auth/sign-in')}>
...
const styles = StyleSheet.create({
  ...
  // Month rows
  monthRow: { backgroundColor: '#f2f2f2', borderRadius: 10, padding: 14, marginBottom: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  monthRowSelected: { borderWidth: 2, borderColor: '#3366FF', backgroundColor: '#f0f4ff' },
  monthLabel: { fontSize: 15, fontWeight: '600' },
  monthMeta: { fontSize: 12, color: '#888', marginTop: 2 },
  chevron: { fontSize: 12, color: '#888' },

  // Expanded month detail
  monthDetail: { backgroundColor: '#f8f8f8', borderRadius: 10, padding: 14, marginBottom: 8 },
  detailSection: { fontSize: 13, fontWeight: '700', color: '#555', marginTop: 10, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 },

  // Summary cards
  summaryRow: { flexDirection: 'row', gap: 8, marginBottom: 4 },
  summaryCard: { flex: 1, backgroundColor: '#fff', borderRadius: 8, padding: 10, alignItems: 'center' },
  ...
  // Expense rows
  expRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#eee' },
  expLogo: { width: 34, height: 34, borderRadius: 17, backgroundColor: '#e0e0e0', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  ...
  noData: { color: '#aaa', fontSize: 13, textAlign: 'center', paddingVertical: 8 },

  // Profile
  avatarContainer: { alignItems: 'center', marginBottom: 16 },
  avatar: { width: 80, height: 80, borderRadius: 40, backgroundColor: '#e0e0e0', alignItems: 'center', justifyContent: 'center' },
  ...
  input: { backgroundColor: '#f2f2f2', borderRadius: 8, padding: 10, fontSize: 14 },
  fullInput: { marginBottom: 14 },
  savedMsg: { color: '#38a169', fontSize: 13, textAlign: 'center', marginBottom: 8 },
  saveBtn: { backgroundColor: '#3366FF', borderRadius: 10, padding: 14, alignItems: 'center', marginTop: 4 },
  saveBtnText: { color: '#fff', fontWeight: 'bold' },
  signOutBtn: { margin: 16, marginTop: 4, backgroundColor: '#fff', borderRadius: 12, padding: 16, alignItems: 'center', borderWidth: 1, borderColor: '#e53e3e' },
